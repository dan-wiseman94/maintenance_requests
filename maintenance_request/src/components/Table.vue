...
const isEditing = (item: Item): boolean => item.id !== undefined && editingId.value === item.id
        ...
        <tr>
          <th v-for="header in headers" :key="header.key">
            ...
            <button
              ...
              class="sort-button"
              :aria-sort="
                directionFor(header) === 'asc'
                  ? 'ascending'
                  : directionFor(header) === 'desc'
                    ? 'descending'
                    : 'none'
              "
              @click="requestSort(header)"
              :disabled="!canSort"
            >
              {{ header.label }}
      ...
      <tbody>
        ...
        <tr v-else v-for="(item, index) in items" :key="item.id ?? index">
          <td v-for="header in headers" :key="header.key">
            ...
            <input
              v-if="isEditing(item) && draft && header.editable !== false"
              v-model="draft[header.key]"
            />

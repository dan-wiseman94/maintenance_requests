...
describe('UsersView', () => {
  ...
  it('retrieves data', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/User?page=1&pageSize=20&orderBy=lastName&desc=false',
    )
  ...
  it('When second load is in flight, Pagination busy = true, then false afterward', async () => {
    const emptyPage = (page: number): PagedResult<User> => ({
      items: [],
      page,
      pageSize: 20,
      totalCount: 40,
    ...
    expect(fetchMock).toHaveBeenLastCalledWith(
      '/api/User?page=2&pageSize=20&orderBy=lastName&desc=false',
    )
